export const homeStyles = {
  page: "min-h-screen bg-lilac p-4",

  layout: "flex gap-4",

  main: "flex-1 rounded-[32px] border border-white/20 bg-white/70 p-6 backdrop-blur-xl",

  divWelcome: "mb-8",

  welcome:
    "rounded-[28px] bg-gradient-to-r from-[#8B5CF6] to-[#E879F9] p-8 text-white shadow-lg",

  title: "text-3xl font-bold tracking-tight",

  presentation: "mt-3 max-w-2xl text-sm text-white/80 leading-relaxed",

  gridChallenges: "mt-10 flex flex-col gap-6",

  cardsGrid: "w-full",
  cardContainer:
    "my-3 rounded-lg w-full h-100 lg:h-80 relative text-primary-foreground",
  imageCard: "rounded-lg w-full h-full object-cover object-center",
  container:
    "flex w-[100%] lg:w-[50%] rounded-lg h-full absolute top-0 right-0 items-center justify-between pr-8 bg-linear-to-l from-text/80 from-80% to-text/0 to-100%",
  detailContainer:
    "flex flex-col h-full justify-between px-4 py-24 lg:py-18 lg:pr-10",
  titleCard: "flex gap-2",
  challengeTitle: " text-2xl",
  statsContainer: "flex gap-2",
  stats: "flex gap-1",
  tag: "font-sans self-center text-text rounded px-1 py-0.5 lg:text-lg text-xs",
};
